...
I've been doing this stuff for some years and usually end up with some mini framework for making life bearable. This time, I started with a clean slate and built it properly with the idea of just putting it out there. So here it is. 

If you want to do some data crunching in Java, you could do worse than start out with this. Sure, you could go for hadoop or any of the other fancy things out there.  But seriously, processing 10GB of wikipedia dump files is not that demanding that you can justify setting up a hadoop cluster and overengineering what would be a three line python file otherwise. With Java the boiler plate code can be a bit overwhelming. 

However, with these classes, you get some nice out of the box features and an easy to use framework with all of the building blocks you need for iterating over and processing blobs of information reliable and fast. This results in a lot less boiler plate code and makes things almost as easy as in python. Combine that with concurrency, decent memory management and a few other things Java coders take for granted and things are starting to look pretty OK.
